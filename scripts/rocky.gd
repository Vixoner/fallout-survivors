...
# Rocky — tanky, slow robot that bombards the player with rocket strikes from
# long range. Same structure as floater.gd (ranged enemy maintaining distance)
# but uses missile_strike.gd from the robot boss instead of homing balls.
...
# Tanky + slow + long range. Missiles in 3-missile volleys with a long cooldown,
# so each barrage is a punishment-window the player has to read and dodge.
@export var max_health: int = 280
@export var move_speed: float = 90.0
@export var preferred_distance: float = 800.0
@export var distance_band: float = 150.0
@export var attack_cooldown: float = 5.0
@export var missile_count: int = 3
@export var missile_target_radius: float = 200.0   # how spread out around the player
@export var missile_interval: float = 0.15         # gap between launches in a single volley
@export var caps_drop_min: int = 4
@export var caps_drop_max: int = 7
...
func _ready() -> void:
	...
	# Stagger initial cooldown so a group of Rockys doesn't volley in sync.
	attack_timer = randf_range(0.0, attack_cooldown * 0.5)
...
func make_champion() -> void:
	...
	missile_count += 1   # 3 → 4 missiles for champion
